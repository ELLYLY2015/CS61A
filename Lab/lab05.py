...
def closer_city(lat, lon, city1, city2):
    ...
    return get_name(city1) if distance(city1, make_city("city3", lat, lon)) == min(distance(city1, make_city("city3", lat, lon)), distance(city2, make_city("city3", lat, lon))) else get_name(city2)
...
def add_trees(t1, t2):
    ...
    b2 = branches(t2)
    if len(b1) < len(b2):
        return tree(label(t1) + label(t2), [add_trees(b1[i], b2[i]) for i in range(len(b1))] + b2[len(b1):])
    else:
        return tree(label(t1) + label(t2), [add_trees(b1[i], b2[i]) for i in range(len(b2))] + b1[len(b2):])
