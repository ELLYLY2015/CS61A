...
def num_sevens(n):
    ...
    return 0 if n < 7 else (n%10 == 7) + num_sevens(n//10)
...
def pingpong(n):
    ...
    """
    "*** YOUR CODE HERE ***"  
    def helper(k, m, count_up):
        if k == n:
            return m
        if k % 7 == 0 or num_sevens(k):
            return helper(k + 1, m - count_up, -count_up)
        return helper(k + 1, m + count_up, count_up)
    return helper(1,1,1)
def count_change(amount):
    ...
    """
    "*** YOUR CODE HERE ***"
    def partition(amount, min):
        if amount == 0:
            return 1
        if amount < min:
            return 0
        return partition(amount - min, min) + partition(amount, min*2)
