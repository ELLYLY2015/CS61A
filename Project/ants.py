...
class Bee(Insect):
    ...
    def action(self, colony):
        ...
        "*** YOUR CODE HERE ***"
        while self.is_scaring > 0 and self.place.name != "Hive":
            destination = self.place.entrance'''
        if self.is_scaring:                    
            if self.place.name != "Hive" and self.place.entrance != None: 
                destination = self.place.entrance                         
            else:
                destination = self.place                          
        else:
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, beehive):
        ...
        p = self.place
        i = 0
        while i <= self.max_range and p != None:
            if p .bees and p.name != 'Hive' and i >= self.min_range:
...
class HungryAnt(Ant):
    ...
    def action(self, colony):
        ...
        if self.digesting > 0:
            ...
            if self.digesting == 0:
                temp = random_or_none(self.place.bees)
                if temp:
                    self.eat_bee(temp)
...
# END Problem 13
    ...
    def __init__(self, armor=1):
        ...
        self.armor = armor
        QueenAnt.number += 1
        self.is_Queen = QueenAnt.number == 1
    ...
    def action(self, colony):
        ...
        if self.is_Queen:
            ...
            while p:
                if p.ant:
                    if not(p.ant in self.is_Double):
                        p.ant.damage *= 2
                        self.is_Double.append(p.ant)
                    if p.ant.is_container:
                        if p.ant.contained_ant:
                            if not(p.ant.contained_ant in self.is_Double):
                                p.ant.contained_ant.damage *= 2
                                self.is_Double.append(p.ant.contained_ant)
                p = p.exit
        else:
            self.armor = 0
            self.place.remove_insect(self)
...
def apply_effect(effect, bee, duration):
    ...
    "*** YOUR CODE HERE ***"
    if effect == make_slow:
        bee.action = make_slow(bee.action, bee)
    elif effect == make_scare and not (bee.is_scared) and not (bee.is_scaring):
        bee.is_scared = True
        bee.is_scaring = True
        bee.action = make_scare(bee.action, bee)
    # END Problem EC
